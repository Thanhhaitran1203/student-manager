  ...
  <nav class="navbar  navbar-expand-lg bg-body-tertiary">
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <form class="d-flex" action="/student?action=search"  method="get">
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <c:forEach items='${requestScope["studentList"]}' var="student">
                  ...
                  <tr class="odd:bg-white even:bg-gray-100">
                    ...
                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${student.getAddress()}</td>
                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${student.getPhoneNumber()}</td>
                 <c:forEach items='${requestScope["class"]}' var="classRoom">
                      <c:if test="${student.getClassId() == classRoom.getId()}">
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${classRoom.getName()}</td>
                      </c:if>
                    </c:forEach>
                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><a href="/student?action=edit&id=${student.getId()}" class="hover:text-indigo-500">Edit</a></td>
                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><a href="/student?action=delete&id=${student.getId()}" class="hover:text-indigo-500">Delete</a></td>
                  </tr>
